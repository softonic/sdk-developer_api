...
class DevelopersApi
{
    ...
    public function __construct(\Softonic\DeveloperApiSdk\ApiClient $apiClient = null)
    {
        if ($apiClient == null) {
            ...
            $apiClient->getConfig()->setHost('http://developer.sftapi.com');
        }
     ...
     * @param int $limit Number of results per page when listing (optional)
     * @param string $search Field to search for (optional)
     * @return Array of \Client\Model\Developer[], HTTP status code, HTTP response headers (array of strings)
     * @throws \Softonic\DeveloperApiSdk\ApiException on non-2xx response
     */
    public function findDeveloperWithHttpInfo($page = null, $limit = null, $search = null)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Client\Model\Developer[]',
                '/developers'
            );

            return array($this->apiClient->getSerializer()->deserialize($response, '\Client\Model\Developer[]', $httpHeader), $statusCode, $httpHeader);
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Client\Model\Developer[]', $e->getResponseHeaders());
